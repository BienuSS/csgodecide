...
<head>
	...
	<link rel="icon" type="image/png" href="img/image.png">
		<title>CSGODECIDE - Football</title>
...
$games = $db->query($query, ['status' => 0, 'limit' => $first, 'firstposition' => $ltmp , 'type_match' => 5]);

$db->bind("status", 0);
$db->bind("type_match", 5);
$max = $db->single("SELECT COUNT(id_match) FROM games WHERE `status` = :status AND CONCAT(`date_match`,' ',`time_match`) > NOW() AND `type_match` = :type_match");
...
if ($first!=0) print ("<a class ='number-site' href=\"football.php?first=" . ($first-$ltmp) . "\" title=\"Poprzednie\"><big>&laquo;</big></a> ");

for ($i=1;$i<=$ile;$i++)
{
   print ("<a class ='number-site' href=\"football.php?first=" . ($i*$ltmp-$ltmp) . "\" title=\"" . ($i*$ltmp-($ltmp-1)) . "-" . ($i*$ltmp) ."\">");
...
if ($akt<$ile) print ("<a class ='number-site' href=\"football.php?first=" . ($first+$ltmp) . "\" title=\"Następne\"><big>&raquo;</big></a>");
